...
This document describes all configuration options and settings available in the Transfer of meter readings to Tatenergosbyt custom integration.
...
### `trans_met_read.example_service`
...
```yaml
service: trans_met_read.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: trans_met_read.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Transfer of meter readings to Tatenergosbyt Alert
...
- [GitHub Issues](https://github.com/Gin-dev-code/hass-my-device/issues) - Report problems
