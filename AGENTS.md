...
- **Repository:** Gin-dev-code/hass-my-device

**Key directories:**

- `custom_components/trans_met_read/` - Main integration code
...
- Integration logs: `custom_components.trans_met_read: debug` in `config/configuration.yaml`
...
- Use the domain `trans_met_read` for all DOMAIN references
- Prefix all integration-specific classes with `TransferOfMeterReadingsToTatenergosbyt`
- Use "Transfer of meter readings to Tatenergosbyt" as the display title
...
- Inherit from platform base + `TransferOfMeterReadingsToTatenergosbytEntity`
...
- `tests/` mirrors `custom_components/trans_met_read/` structure
